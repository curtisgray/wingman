...
import { IconPlaneDeparture, IconPlaneOff } from '@tabler/icons-react';
...
export default function ModelListing ({ onSelect = () => { }, isDisabled: disabled = false }: Props) {
    const listSize = 10;
        ...
        if (Vendors[model.vendor].isDownloadable) {
            if (model.isInferable) {
                return <div className="ml-auto text-sky-400">
                    <IconPlaneDeparture size={23} data-tooltip-id="is-inferable" data-tooltip-content="Cleared for takeoff" />
                    <Tooltip id="is-inferable" />
               </div>;
            } else {
                return <div className="ml-auto text-neutral-400">
                    <IconPlaneOff size={23} data-tooltip-id="is-not-inferable" data-tooltip-content="Not cleared for takeoff" />
